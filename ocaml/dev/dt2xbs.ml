...
Bug reports welcome! stabler@ucla.edu

requires noOther from dtState.ml
requires splitThird from dt2bts.ml
*)

type index = N | I of int;;
type xbTree =   (* for branching categories, we use bool first? to tell if head is first subtree *)
    X01 of string * string list                      (* (cat,ph) *)
  | X02 of string * bool * xbTree * xbTree           (* (cat,first?,subhead1,subhead2) for complex heads *)
  ...
  | XP0 of string * index * string list              (* (cat,index,ph) for 0-ary XP = traces, lexical XPs *)
  | XP1 of string * index * xbTree                   (* (cat,index,subtree) for unary XP *)
  | XP2 of string * index * bool * xbTree * xbTree   (* (cat,index,first?,subtree1,subtree2) for binary XP *)

let rec xb2t = function
  |  X01 (cat,string) ->
      T(cat,[T(spacedstring2 string,[])])
  | X02 (cat,_,t1,t2) ->
    T(cat,[xb2t t1;xb2t t2])
  | X11 (cat,t1) ->
    T(cat^"'",[xb2t t1])
  | X12 (cat,_,t1,t2) ->
    T(cat^"'",[xb2t t1;xb2t t2])
  | XP0 (cat,N,string) ->
    T(cat^"P",[T(joinStrings "" string,[])])
  | XP0 (cat,I n,string) ->
    T(cat^"P("^(string_of_int n)^")",[T(joinStrings "" string,[])])
  | XP1 (cat,N,t1) ->
    T(cat^"P",[xb2t t1])
  | XP1 (cat,I n,t1) ->
    T(cat^"P("^(string_of_int n)^")",[xb2t t1])
  | XP2 (cat,N,_,t1,t2) ->
    T(cat^"P",[xb2t t1;xb2t t2])
  | XP2 (cat,I n,_,t1,t2) ->
    T(cat^"P("^(string_of_int n)^")",[xb2t t1;xb2t t2])
...
let traceOf = function
  | XP0(cat,I i,_) -> XP0(cat,I i,[])
  | XP1(cat,I i,_) -> XP0(cat,I i,[])
  | XP2(cat,I i,f,t1,t2) -> XP0(cat,I i,[])
...
let insertIndex i = function
  | XP0(cat,N,st) -> XP0(cat,I i,st)
  | XP1(cat,N,t) -> XP1(cat,I i,t)
  | XP2(cat,N,f,t1,t2) -> XP2(cat,I i,f,t1,t2)
  | _ -> failwith "insertIndex error"

let rec dt2xbs0 = function
  | L (strings,Cat cat::fs) -> 
	([Cat cat::fs],true,[XP0(cat,N,strings)])
  | L (strings,fs) -> 
    ...
    let (fs2,_,xb2,xbs2) = splitThird (dt2xbs0 d2) in
	  ...
	  (((Sel f1)::Cat c::fs1)::movers1,true,((Cat f2)::fs2)::movers2) when f1=f2 -> 
	    ...
	    then ((Cat c::fs1)::movers1@movers2,
		  false,
		  XP2(c,N,true,xb1,xb2)::xbs1@xbs2) (* merge 1, XP *)
	    else 
	      let i = myindex#next in 
	      let newxb2 = insertIndex i xb2 in
		((Cat c::fs1)::fs2::movers1@movers2,
		 false,
		 XP2(c,N,true,xb1,XP0(f2,I i,[]))::newxb2::xbs1@xbs2) (* merge 3, XP *)
	| (((Sel f1)::Cat c::fs1)::movers1,false,((Cat f2)::fs2)::movers2) when f1=f2 -> 
	  if fs2 = []
	  then ((Cat c::fs1)::movers1@movers2,
		false,
		XP2(c,N,false,xb2,xb1)::xbs1@xbs2) (* merge 2, XP *)
	  else 
	    let i = myindex#next in 
	    let newxb2 = insertIndex i xb2 in
	      ((Cat c::fs1)::fs2::movers1@movers2,
	       false,
	       XP2(c,N,false,XP0(f2,I i,[]),xb1)::newxb2::xbs1@xbs2) (* merge 3, XP *)
	  ...
	  then (fs1::movers1@movers2,
	    ...
	    let newxb2 = insertIndex i xb2 in
	      (fs1::fs2::movers1@movers2,
	       false,
	       X12(catOfFs fs1,true,xb1,XP0(f2,I i,[]))::newxb2::xbs1@xbs2) (* merge 3, X' *)
	| (((Sel f1)::fs1)::movers1,false,((Cat f2)::fs2)::movers2) when f1=f2 -> 
	  ...
	  then (fs1::movers1@movers2,
	    ...
	    let newxb2 = insertIndex i xb2 in
	      (fs1::fs2::movers1@movers2,
	       false,
	       X12(catOfFs fs1,false,XP0(f2,I i,[]),xb1)::newxb2::xbs1@xbs2) (* merge 3, X' *)
    ...
    let (fs,_,xb,xbs) = splitThird (dt2xbs0 d) in
	  ...
	  ((Pos f)::Cat c::fs)::movers -> 
	    let (newMovers,newTrees,movingTree) = selectUniqueTripleX f movers xbs in
	      ((Cat c::fs)::newMovers,false,XP2(c,N,false,movingTree,xb)::newTrees)      (* move, XP *)
	| ((Pos f)::fs)::movers -> 
	    let (newMovers,newTrees,movingTree) = selectUniqueTripleX f movers xbs in
	      (fs::newMovers,false,X12(catOfFs fs,false,movingTree,xb)::newTrees)        (* move, X' *)
	| _ -> failwith "dt2xbs0 move error"
