...
scan :: [String] -> [String] -> V.Vector [LexTree] -> VMvIdx -> [Deriv]
  -> [Deriv]
...
merge1 :: LexTrees -> [String] -> [LexTree] -> Int -> IdxCat -> Movers -> VMvIdx
  -> [Deriv] -> [Deriv]
merge1 lts input terms i ((_,m),(hx,mx)) empty_m empty_mx sofar =
  if terms /= []
  then
...
merge2 :: LexTrees -> [String] -> [LexTree] -> Int -> IdxCat -> Movers ->VMvIdx
  -> [Deriv] -> [Deriv]
merge2 lts input nonterms i ((_,m),(hx,mx)) empty_m empty_mx sofar =
  if nonterms /= []
  then
...
insertNewDerivs p new_p q dq ((input,ics):more) =
  case ics of
  ...
  then False
    ...
    then
      ...
      then
        ...
        if new_p > min
        then let new_parses = insertNewDerivs p new_p new_iq pop_dq xs in
