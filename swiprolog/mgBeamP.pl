...
%:- [mgxx].
:- [latex_tree].
%:- lexBuild(LexTree), latex_tree(LexTree), wish_tree(LexTree).
:- lexBuild(LexTree), parse(LexTree,[which,wine,the,queen,prefers],T), dt2stt(T,STT), btfy_stt(STT,B), latex_tree(STT), wish_tree(STT).
%:- lexBuild(LexTree), parse(LexTree,['Sue',knows,the,king,knows,which,beer,'John',prefers],T), dt2bt(T,X), btfy_bt(X,B), latex_tree(X), wish_tree(X).
%:- lexBuild(LexTree), parse(LexTree,['Sue',knows,the,king,knows,which,beer,'John',prefers],T), btfy(T,B), latex_tree(B), wish_tree(B).
%:- lexBuild(LexTree), parse(LexTree,['Sue',knows,the,king,knows,which,beer,'John',prefers],T), dt2bt(T,X), btfy_bt(X,B), latex_tree(B), wish_tree(B).
...
%       btfy(T,D), wish_tree(D), sleep(5),
%       dt2bt(T,BT), btfy_bt(BT,BBT), wish_tree(BBT), sleep(5),
%       dt2stt(T,STT), btfy_stt(STT,BSTT), wish_tree(BSTT), sleep(5),
