% file: mgBeamPpa.pl.
%   mgBeamPp.pl adds persistent features to the basic parser mgBeamP.pl
%   This file adds adjunction to mgBeamPp.pl
% TODO: Add left,right adjoing *mover*
:- [library(heaps)].
...
extendBeam(LexTs,Beam0,D) :-
    ...
    ( success(In,Q0), D=A0
    ; get_from_heap(Q0,_,(R/Ts,Anc,TI,Movers,Ancs,MIs,A),Q), % pop leftmost cat
      findall(Parse,(member(T,Ts),infer(T,Anc,TI,Movers,Ancs,MIs,A,LexTs,(In,Q,A0),Parse)
		    ;R=cat(F),adjoin(F,Ts,Anc,TI,Movers,Ancs,MIs,A,LexTs,(In,Q,A0),Parse)
		    ),New),
      length(New,NumberOfOptions), 
      ( NumberOfOptions>0,
	P is (1/NumberOfOptions)*P0, % uniform probability over next steps
	P < -0.00001 -> % Simple pruning rule: improbability bound (cf Roark'01)
...
% STEPS:  adjoin(F,Ts,Anc,I,Movers,MIs,Derivation,Lex,(Input0,Queue0),(Input,Queue))
adjoin(F,FTs,Anc,TI,Ms0,Ancs0,MIs0,x/[B,C],LexTs,(In,Q0,A),(In,Q,A)) :-  % UNADJOIN
    append01(TI,TI0,TI1), % extend tree index TI with 0 and 1
    (  memberOnce(ra(F)/CTs,LexTs),
       add_to_heap(Q0,TI0,(cat(F)/FTs,Anc,TI0,Ms0,Ancs0,MIs0,C),Q1),
       least(TI1,MIs0,Least),
       add_to_heap(Q1,Least,(ra(F)/CTs,[ra(F)],TI1,[],[],[],B),Q)
    ;  memberOnce(la(F)/CTs,LexTs),
       least(TI1,MIs0,Least),
       add_to_heap(Q0,Least,(cat(F)/FTs,Anc,TI1,Ms0,Ancs0,MIs0,C),Q1),
       add_to_heap(Q1,TI0,(la(F)/CTs,[la(F)],TI0,[],[],[],B),Q)
    ).
...
% Tree drawing tools, lexical tree builder, examples
:- [pp_tree,wish_tree,lexBuild,beautify,dt2bt,dt2stt,dt2xb].
%:- [mg0t0].
%:- lexBuild(LexT), parse(LexT,[these,'PL','KING','PRES','PREFER',this,'SG','BEER'],T),
:- [mg0t1].
%:- lexBuild(L), wish_tree(L).
%:- lexBuild(LexT), parse(LexT,['JOHN','KNOW',this,'SG','KING','PRES','LAUGH'],T),
%:- lexBuild(LexT), parse(LexT,['JOHN','PRES','KNOW',this,'SG','KING','PRES','LAUGH'],T),
%:- lexBuild(LexT), parse(LexT,[these,'PL','KING','PRES','PREFER',this,'SG','BEER'],T),
%:- lexBuild(LexT), parse(LexT,['MARY','PRES','KNOW','JOHN','PRES','PREFER',this,'SG','BEER'],T),
%:- lexBuild(LexT), parse(LexT,['MARY','PRES','KNOW',which,'SG','BEER','JOHN','PRES','PREFER'],T),
%:- lexBuild(LexT), parse(LexT,['MARY','PRES','KNOW',which,'SG','DARK','BEER','JOHN','PRES','PREFER'],T),
%:- lexBuild(LexT), parse(LexT,[which,'SG','DARK','BEER','JOHN','PRES','PREFER',on,'SUNDAY'],T),
%:- lexBuild(LexT), parse(LexT,['JOHN','PRES','KNOW',these,'PL','KING','PRES','PREFER',
%                               this,'SG','BEER'],T),
%:- lexBuild(LexT), parse(LexT,['JOHN','PRES','KNOW',which,'SG','BEER',these,'PL','KING',
%                               'PRES','PREFER'],T),
%:- lexBuild(LexT), parse(LexT,[these,'PL','KING','PRES','PREFER',this,'SG','BEER',tomorrow],T),
%:- lexBuild(LexT), parse(LexT,[these,'PL','KING','PRES','REALLY','PREFER',this,'SG','BEER'],T),
%:- lexBuild(LexT), parse(LexT,[these,'PL','KING','PRES','REALLY','PREFER',this,'SG',
%			       'DARK','DARK','BEER'],T),
%:- lexBuild(LexT), parse(LexT,[these,'PL','KING','PRES','REALLY','PREFER',this,'SG',
%                               'DARK','DARK','BEER',tomorrow],T),
%:- lexBuild(LexT), parse(LexT,[these,'PL','KING','PRES','REALLY','PREFER',this,'SG',
%                               'DARK','DARK','BEER',on,'SUNDAY'],T),
% Out of global stack on this next one:
%	optimize probability distribution? bigger machine? optimize code? Yes!
:- lexBuild(LexT), parse(LexT,['JOHN','PRES','KNOW',which,'SG','DARK','DARK','BEER',these,
                               'PL','KING','PRES','REALLY','PREFER',on,'SUNDAY'],T),
       btfy(T,D), wish_tree(D), sleep(3),
       dt2bt(T,BT), btfy_bt(BT,BBT), wish_tree(BBT), sleep(3),
       dt2stt(T,STT), btfy_stt(STT,BSTT), wish_tree(BSTT), sleep(3),
