...
dt2xb(_/[A,B],(Al,Ms),Cat,BP/[BP/BTB,AP/BTA]) :- %right adjoin
	 dt2xb(A,([ra(F)],[]),_BCat,_/BTA),
	 dt2xb(B,([sel(F)|Al],Ms),Cat,_/BTB),
	 atomic_concat('~r',F,ACat),
	 catMax(ACat,AP), catMax(Cat,BP).
dt2xb(_/[A,B],(Al,Ms),Cat,BP/[AP/BTA,BP/BTB]) :- %left adjoin
	 dt2xb(A,([la(F)],[]),_BCat,_/BTA),
	 dt2xb(B,(Al,Ms),Cat,_/BTB),
	 atomic_concat('~r',F,ACat),
	 catMax(ACat,AP), catMax(F,BP).
*/

dt2xb(_/[A/[],B],(Al,Ms),Cat,CatMid/[XBA,BP/XBB]) :- %mrg1
	 dt2xb(B,([cat(F)],Ms),BCat,_/XBB), % B first, to determine if mrg1 is correct case
	 dt2xb(A/[],([sel(F)|Al],[]),Cat,XBA),
	 catMid(Cat,CatMid), catMax(BCat,BP).
dt2xb(_/[A/[A1|L1],B],(Al,Ms12),Cat,CatMid/[BP/XBB,XBA]) :- %mrg2
	 dt2xb(B,([cat(F)],Ms2),BCat,_/XBB),
	 dt2xb(A/[A1|L1],([sel(F)|Al],Ms1),Cat,XBA),
	 append(Ms1,Ms2,Ms12), smc_xb(Ms12),  catMid(Cat,CatMid), catMax(BCat,BP).
dt2xb(_/[A/[],B],(Al,Ms12),Cat,CatMid/[XBA,Ti/[]]) :- %mrg3a
	 dt2xb(B,([cat(F),G|Be],Ms2),BCat,_/XBB),
	 dt2xb(A/[],([sel(F)|Al],Ms1),Cat,XBA),
	 append([ ([G|Be],BPi/XBB) |Ms1], Ms2 , Ms12), smc_xb(Ms12), % INSERT INTO MOVERS
	 %catOf(Al,Cat), 
	 catMid(Cat,CatMid), catMax(BCat,BP), BPi=..[BP,Trace], Ti=..[t,Trace].
...
dt2xb(o/[A],(Al,Ms),Cat,CatMid/[Spec,XBA]) :-
	 ...
	 catMid(Cat,CatMid).
dt2xb((W:Fs)/[],(Fs,[]),Cat,Cat/[W/[]]) :- catOf(Fs,Cat).

catOf([ra(C)|_],D) :- !, atomic_concat('~r',C,D).
catOf([la(C)|_],D) :- !, atomic_concat('~l',C,D).
catOf([cat(C)|_],D) :- !, C=D.
catOf([_|Fs],C) :- catOf(Fs,C).
